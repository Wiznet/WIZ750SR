...
static bool mbTCPGet(uint8_t sock, uint8_t ** ppucMBTCPFrame, uint16_t * usTCPLength )
{
  ...
  if( len > 0 )
  {
    if (network_info->working_mode == UDP_MODE) {
      usTCPBufPos = recvfrom(SOCK_DATA, aucTCPBuf, len, peerip, &peerport);
      ctlsocket(sock, CS_CLR_INTERRUPT, (void *)&reg_val);
 
      if(memcmp(peerip, network_info->remote_ip, sizeof(peerip)) || network_info->remote_port != peerport)
      {
        network_info->remote_ip[0] = peerip[0];
        network_info->remote_ip[1] = peerip[1];
        network_info->remote_ip[2] = peerip[2];
        network_info->remote_ip[3] = peerip[3];
        network_info->remote_port = peerport;
      }
    }
    else {
